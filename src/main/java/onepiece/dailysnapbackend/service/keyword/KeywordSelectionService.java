...
//    Optional<Keyword> optionalAdminKeyword = keywordRepository.findByKeywordCategoryAndProvidedDate(ADMIN_SET, today);
...
//    Keyword unusedKeyword = keywordRepository.findFirstByKeywordCategoryAndUsedFalse(selectedCategory)
//        .orElseGet(() -> {
//          log.warn("사용 가능한 키워드 없음, OpenAI로 새 키워드 생성: category={}", selectedCategory);
//          openAIKeywordService.generateKeywords(selectedCategory);
//          return keywordRepository.findFirstByKeywordCategoryAndUsedFalse(selectedCategory)
//              .orElseThrow(() -> {
